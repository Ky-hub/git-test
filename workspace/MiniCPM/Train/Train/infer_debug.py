...
# -*- coding: utf-8 -*-
"""
MiniCPM-O-4.5 推理调试脚本 (infer_debug.py) —— v8 完整修复版
放置于 Train 目录，与 MiniCPMO45 同级。

修复记录：
  - v8: 修复 Token2wav.stream() hook 参数名不匹配（改用 *args, **kwargs 透传）
         修复 processor 缺失 pixel_values / tgt_sizes 导致的 KeyError
         无图像时 pixel_values 传 [[]]、tgt_sizes 传 [[]] 避免迭代报错
"""
...
# ==================== Monkey-patch Hook（只打印，不修改逻辑） ====================

def install_hooks(model, tokenizer):
    device = model.device
    ...
    def _hooked_generate_speech(outputs, tts_bound, tts_proj_layer, audio_prompt,
        ...
        if isinstance(result, np.ndarray):
            print(f">>> 返回波形: {len(result)} samples, {len(result)/24000:.2f}s @ 24kHz")
        elif isinstance(result, torch.Tensor):
            print(f">>> 返回波形 tensor: {tuple(result.shape)}")
        else:
    ...
    if (hasattr(model, 'tts') and model.tts is not None and
        ...
        if hasattr(tokenizer_obj, 'stream'):
            ...
            def _hooked_t2w_stream(*args, **kwargs):
                # 从 args/kwargs 中安全提取 token 序列用于打印
...
def analyze_input_structure(model, msgs, use_tts_template=True):
    ...
    # 4. Embedding 替换后的 shape
    # 关键：pixel_values / tgt_sizes 不能传 None，必须传 [[]] 避免 get_vision_embedding 迭代报错
    pixel_values = inputs.get("pixel_values")
    if pixel_values is None:
        pixel_values = [[]]  # batch=1, 0张图
    ...
    print(f"    其中 audio_bounds 对应的位置已被 Audio Encoder 输出的 Embedding 替换")

    # 5. Loss Mask 建议
    print("\n>>> 建议 Loss Mask 规则:")
    print("    - System/User 段 (prompt): mask = 0")
    print("    - Assistant 生成段: mask = 1")
    print("    - 特殊结构 token (<|tts_bos|>, <|tts_eos|>): mask = 1 (让模型学会自触发)")
...
def main():
    ...
    # 4. 安装 Hook
    install_hooks(model, model.processor.tokenizer)
