#!/usr/bin/env python3
"""MiniCPM-o 音色克隆批量测试 Gateway（输入音频与参考音色一致）

核心逻辑：
- voice_dir: 音频目录（每个音频既作为输入内容，又作为参考音色）
- 每个音频独立成一个任务：输入音频 = 参考音色，模型复述该音频内容
"""
...
class BatchConfigTemplate:
    def __init__(self, raw: Dict[str, Any]):
        ...
        self.voice_dir = raw.get("voice_dir", "")          # 参考/输入音频目录（同一目录）
...
class BatchJob:
    def __init__(self, job_id: str, audio_dir: str, output_dir: str,
                 system_prompt: str, voice_config: Dict[str, Any],
                 manifest_path: Optional[str] = None):
        self.id = job_id
        self.audio_dir = audio_dir        # 参考/输入音频目录（同一目录）
...
app = FastAPI(
    ...
    description="音色克隆批量测试（输入音频与参考音色一致，模型复述内容）",
    version="2.3.0-batch-voice-clone",
    lifespan=lifespan,
    docs_url=None,
    redoc_url=None,
)
...
# ============================================================
# 批量任务 API（每个音频 = 输入 + 参考音色）
# ============================================================

@app.post("/api/batch/jobs")
async def create_batch_job(
    background_tasks: BackgroundTasks,
    voice_dir: str = Form(..., description="参考/输入音频目录（每个音频既作为输入内容又作为参考音色）"),
    voice_paths: Optional[str] = Form(None, description="额外音频文件路径（逗号分隔，可选）"),
    selected_voice_files: Optional[str] = Form(None, description="选中的音频文件（逗号分隔的相对/绝对路径，不传则跑全部）"),
    output_dir: Optional[str] = Form(None, description="输出目录（服务器本地绝对路径，默认自动生成）"),
    system_prompt: str = Form(default="请仔细听取用户发送的音频内容，识别其中的文字，并用自然流畅的中文语音将这段文字完整复述一遍。"),
    ...
    if not unique_voices:
        raise HTTPException(status_code=400, detail="未找到任何音频文件，请检查 voice_dir、voice_paths 或 selected_voice_files")

    # 创建任务：每个音频既作为输入，又作为参考音色
    ...
    for vpath in unique_voices:
        ...
        job.tasks.append({
            "voice_path": vpath,
            "voice_name": vname,
            "input_file": vpath,      # 输入音频 = 参考音色（同一文件）
            "input_name": vname,
        })
...
async def run_batch_job(job: BatchJob):
    ...
    for idx, task in enumerate(job.tasks):
        ...
        try:
            ...
            # 输出命名：基于音频文件名
            safe_name = re.sub(r'[^a-zA-Z0-9_.\-]', '_', os.path.splitext(task["voice_name"])[0])
            out_name = f"{safe_name}_clone_out.wav"
            ...
            if success and out_audio_b64:
                ...
                entry = {
                    "index": idx,
                    "input_audio": task["input_name"],           # 输入音频文件名（与参考音色相同）
                    "input_path": task["input_file"],            # 输入音频完整路径
            ...
            logger.error(f"[Batch {job.id}] 失败 {task['voice_name']}: {e}", exc_info=True)
            entry = {
                "index": idx,
                "input_audio": task["input_name"],
                "input_path": task["input_file"],
...
def _write_manifest(job: BatchJob, entries: List[Dict[str, Any]]):
    ...
    manifest = {
        "job_id": job.id,
        "audio_dir": job.audio_dir,          # 参考/输入音频目录
...
def main():
    ...
    logger.info(f"Starting Batch Voice Gateway v2.3 on {args.host}:{args.port}")
